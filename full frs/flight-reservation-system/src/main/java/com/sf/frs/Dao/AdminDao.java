...
//	session=sessionFactory.openSession();
//	transaction=session.beginTransaction();
//	Query q8=session.createQuery("from ScheduleBean where scheduleID=:schid ");
//	q8.setParameter("schid", scheduleid);
//	transaction.commit();
//	
//	scheduleBean=(ScheduleBean)q8.list();
//	return scheduleBean;
//	
//} 
...
public ScheduleBean getScheduleById(int scheduleid) {
    ...
    try {
        ...
        Query q8 = session.createQuery("from ScheduleBean where scheduleID=:schid ");
        q8.setParameter("schid", scheduleid);
        List<ScheduleBean> results = q8.list();
        if (!results.isEmpty()) {
            scheduleBean = results.get(0);
        }
        transaction.commit();
    } catch (Exception e) {
        if (transaction != null) {
